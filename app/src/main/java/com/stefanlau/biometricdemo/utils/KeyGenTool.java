...
public class KeyGenTool {
    ...
    private static final String TRANSFORMATION = KEY_ALGORITHM + "/" + KEY_BLOCK_MODE + "/" + KEY_PADDING; // "AES/CBC/PKCS7Padding"
     ...
     * 获取解密的cipher
     *
     * @param initializeVector 加密Cipher的初始向量(AES加密中 以CBC模式加密需要一个初始的数据块，解密时同样需要这个初始块)
     * @return
     */
    @RequiresApi(api = Build.VERSION_CODES.M)
    ...
    private SecretKey getKey() throws Exception {
        ...
        keyStore.load(null);
        if (keyStore.isKeyEntry(KEY_ALIAS)) { //读取秘钥库中key
            ...
            KeyGenParameterSpec spec = new KeyGenParameterSpec
                    ...
                    .setUserAuthenticationRequired(true) //这个设置为true，表示这个key必须是通过了用户认证才可以使用
                    .build();
