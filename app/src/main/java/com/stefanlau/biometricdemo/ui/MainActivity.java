package com.stefanlau.biometricdemo.ui;
...
import com.stefanlau.biometricdemo.R;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.tv_open_finger_login: //开通指纹登录
                /**
                 * 先要用户输入登录密码
                 * 自行处理获取用户输入的密码
                 * 另外自行做加密保护
    ...
    private void openFingerLogin(final String pwd) {
        if (mManager.isBiometricPromptEnable()) {
            mManager.authenticate(false, new OnBiometricIdentifyCallback() {
                @Override
    ...
    private void fingerLogin() {
        if (mManager.isBiometricPromptEnable()) {
            mManager.authenticate(true, new OnBiometricIdentifyCallback() {
                @Override
                public void onUsePassword() {
                }
